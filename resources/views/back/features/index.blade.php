<x-app-layout>
                  ...
                  <table class="table table-striped"  id="table-1" >
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          <!-- Status Update Form -->
                          <form action="{{ route('feature.show', ['feature' => $list->id]) }}" method="GET">
                            @csrf
                        ...
                        <td>{{ $list->order }}</td>
                        <td>
                          <form action="{{ route('feature.edit', ['feature' => $list->id]) }}" method="GET" class="d-inline">
                            @csrf
                            <button type="submit" class="edit badge badge-primary btn btn-primary"><i class="fas fa-edit" ></i></button>
                          </form>

                          <form id="deleteForm{{ $list->id }}" action="{{ route('feature.destroy', ['feature' => $list->id]) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
  ...
  <script>
    ...
    function confirmDelete(blogId) {
      if (confirm('Are you sure you want to delete this feature entry?')) {
